...
class MolConv(MessagePassing):
    def message(self, x_j: Tensor, edge_attr: OptTensor, edge_weight: OptTensor = None) -> Tensor:
        #print(f"[DEBUG] x_j.device: {x_j.device}")
        if edge_weight is not None:
            #print(f"[DEBUG] edge_weight.device: {edge_weight.device}")
            edge_weight = edge_weight.to(x_j.device)
        if edge_attr is not None:
            #print(f"[DEBUG] edge_attr.device: {edge_attr.device}")
            edge_attr = edge_attr.to(x_j.device)
...
class WeightedGCNConv(MolConv):
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_attr: OptTensor = None, edge_weight: OptTensor = None) -> Tensor:
        ...
        if edge_attr is not None:
            edge_attr = edge_attr.to(self.device)
        '''
        print(f"[DEBUG] inside GraphConvLayer.forward()")
        print(f"  device = {self.device}")
        print(f"  x.device = {x.device}")
        print(f"  edge_index.device = {edge_index.device}")
        print(f"  edge_weight.device = {edge_weight.device if edge_weight is not None else 'None'}")
        print(f"  edge_attr.device = {edge_attr.device if edge_attr is not None else 'None'}")
        '''
