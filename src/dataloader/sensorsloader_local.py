...
def load_credentials():
    with open('src/dataloader/Credenciales.txt') as file:
...
def load_device_repository(metodo):
    dfRepos = pd.read_csv('src/dataloader/CLIENT/Repositorios.csv')
    deviceType = "DEVICE"
...
def manage_null_data(df, save_file="src/dataloader/CLIENT/" + os.environ['CLIENT'] + "/fix_values.json"):
...
def get_sensordata(N, Nulo, Metodo):
    ...
    #### Test de nulos  ####
    #with open('src/dataloader/ETL/TEST/datos_5.json', 'r') as file:
    ...
    if os.environ["TASK"] != "re-train":
        df_datos.to_csv('src/dataloader/ETL/datos.csv', index=False)
        ...
        #### Test de nulos  ####
        #with open('src/dataloader/ETL/TEST/datos_5_postnull.json', 'w') as file:
        ...
        newdf_datos.to_csv('src/dataloader/ETL/datosNaN.csv', index=False)
        ...
        # Create folder
        os.makedirs(f"src/data/{os.environ['CLIENT']}", exist_ok=True)
        # Save DataFrame as CSV
        df_datos.to_csv(f"src/data/{os.environ['CLIENT']}/data.csv", index=False)
        return "ok"